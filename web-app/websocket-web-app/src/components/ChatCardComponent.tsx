...
function ChatCardComponent(props: ChatCardComponentProps) {
    ...
    return (
        <div className="p-2 dark:bg-gray-700 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200" onClick={handleSelectUser}>
            <div className="flex items-center space-x-2">
                <div className="shrink-0 relative">
                    ...
                    />
                    <span className={`absolute bottom-0 right-0 w-2 h-2 sm:w-3 sm:h-3 rounded-full border-2 border-white dark:border-gray-700 ${props.user.status === 'ONLINE' ? 'bg-green-500' : 'bg-gray-400'
                        }`}></span>
                </div>
                <div className="min-w-0 flex-1 overflow-hidden">
                    <div className="flex justify-between items-center space-x-2">
                        <p className="text-xs sm:text-sm font-semibold text-gray-900 dark:text-white truncate">{props.user.fullName}</p>
